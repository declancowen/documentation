...
### get_posts_list / get_recipes_list / get_collections_list

Dedicated listing functions for each content type with sorting and pagination.

```sql
get_posts_list(p_limit, p_offset, p_sort, p_current_user_id) → json
get_recipes_list(p_limit, p_offset, p_sort, p_current_user_id) → json
get_collections_list(p_limit, p_offset, p_current_user_id) → json
```

<Note>
  `get_recipes_list` is defined but not yet implemented. Recipe listing currently uses the `recipe_with_stats` view directly.
</Note>
...
Similar atomic operations for posts and soft-delete operations.
...
Gets saved items not in any collection.
