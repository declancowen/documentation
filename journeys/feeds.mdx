...
## Creators / Featured Users

`GET /api/creators` — lists featured/active creators with sorting.
- RPC: `get_creators_list` with `p_limit`, `p_offset`, `p_sort`
- Displayed via `CreatorCard` component
- Query key: `queryKeys.creators.list(filters)`
...
| `get_creators_list` | `p_limit`, `p_offset`, `p_sort` | JSON | Featured/active creators |

<Note>
There is no standalone `get_recipes_list` RPC function. Recipe listing is handled through the discover content list or via direct Supabase queries in the API route (`/api/recipes/list`).
</Note>
