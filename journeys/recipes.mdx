...
Recipes are the atomic unit of Recipe Room — a complete set of Fujifilm camera settings with sample images, tags, and optional Lightroom adjustments. Users create, view, edit, and soft-delete recipes.
...
<Steps>
  ...
  <Step title="Submit">
    Images uploaded first → then `POST /api/recipes` with all data.
  </Step>
</Steps>
...
Owner taps edit → `app/recipe/[id]/edit.tsx` → pre-populated form (fetched via `useRecipe(id)`) → can modify all fields including images → `PUT /api/recipes/[id]` (calls `update_recipe` RPC). Uses `KeyboardAvoidingView` with platform-specific behavior.
...
| Hook | Purpose |
|------|---------|
| `useRecipes` | React Query hook for recipe list/CRUD (infinite query, mutations) |
...
| `useDeleteRecipe` | `supabase.rpc('delete_recipe', {...})` | Soft delete |
...
| `DELETE` | `/api/recipes/[id]` | Soft delete recipe (calls `delete_recipe` RPC) |
...
<AccordionGroup>
  ...
  <Accordion title="delete_recipe">
    ...
    **Returns:** `TABLE(id, image_urls[], deleted_at)`
    ...
    Soft delete: sets `is_deleted = true`. Returns image URLs for storage cleanup.
  </Accordion>
